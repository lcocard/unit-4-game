...
$(document).ready(function() {
  ...
  $(".attkbtn").click(function() {
    if ($("#dynamicID").length > 0) {
      // alert("HI12");
      console.log("Test click attk bttn: #dynamicID exists!!!");
    } else {
      // alert("HI13");
  ...
  $(".grid_deff").click(function() {
    if (Game_Over === 2) {
      alert(
        "You've been defetead by " +
          dRealName +
          "... GAME OVER!!!\n\nClick OK to play again."
      );
      document.body.addEventListener("click", window.location.reload());
    } else {
      console.log("Game Over val1 = " + Game_Over);
    }
    // $(".No_Enemy_Here").empty();
    i_enemy++;
    if (aHP <= 0) {
    ...
    $(".attkbtn").click(function() {
      ...
      if (!(aHP < 1) || dHP < 1) {
        dHP = dHP - aAP;
        $(".You_Attacked").html(
          "You attacked " + dRealName + " for " + aAP + " damage."
        );
        $("." + yourD).html("<span>" + dHP + "</span>");
        aHP = aHP - dCAP;

        $(".Attacked_You_Back").html(
          dRealName + " attacked you back for " + dCAP + " damage. "
        );
      ...
      if (dHP < 0) {
        ...
        $(".You_Have_Defeated_GO").html(
          "You have defeated " +
            dRealName +
            ", you can chose to fight another enemy."
        );
        $("#defender-char").empty();
        $("#dynamicID").empty();
        $(".attkbtn").hide();
